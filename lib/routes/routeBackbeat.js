...
function _putConditionalTags(request, response, locations, log, cb) {
    return async.waterfall([
        next => _getLastModified(locations, log, next),
        (lastModified, next) => {
            const ifUnmodifiedSince = request.headers['if-unmodified-since'];
            if (ifUnmodifiedSince !== lastModified) {
                log.info('object has been modified, not putting tags', {
                    method: '_putConditionalTags',
                    ifUnmodifiedSince,
                    lastModified,
                });
                return next();
            ...
            return handleTaggingOperation(
                request, response, 'Put', undefined, log, next);
        },
    ], err => {
        if (err) {
            log.error('put tags if-unmodified-since operation failed', {
                method: '_putConditionalTags',
                error: err,
            });
            return cb(err);
        }
        log.debug('put tags if-unmodified-since operation successful');
        return cb();
    });
}

function _handleConditionalDelete(request, response, locations, log, cb) {
    const storageClass = request.headers['x-scal-storage-class'];
    const location = locationConstraints[storageClass];
    // Azure supports a conditional delete operation, while other clouds don't.
    if (location && location.type === 'azure') {
        return _azureConditionalDelete(request, response, log, cb);
    }
    return _putConditionalTags(request, response, locations, log, cb);
}
...
function batchDelete(request, response, log, callback) {
    return _getRequestPayload(request, (err, payload) => {
        ...
        if (request.headers['if-unmodified-since']) {
            return _handleConditionalDelete(
