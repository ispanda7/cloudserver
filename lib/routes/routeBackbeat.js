...
function headObject(request, response, log, callback) {
    ...
    try {
        ...
        return callback(errors.InvalidRequest.customizeDescription(msg));
    }
    ...
    return dataClient.head(locations, reqUids, (err, data) => {
        ...
        const dataRetrievalInfo = { lastModified: data.LastModified };
        return _respond(response, dataRetrievalInfo, log, callback);
    });
}
...
function _batchDeleteIfUnmodifiedSince(
    request, response, locations, log, callback) {
        return async.waterfall([
            next => {
                const reqUids = log.getSerializedUids();
                return dataClient.head(locations, reqUids, next);
            },
            (data, next) => {
                const { headers } = request;
                if (headers['if-unmodified-since'] !== data.LastModified) {
                    return next(null);
                }
            ...
            if (err) {
                log.error('batch delete if-unmodified-since operation failed', {
                    method: '_batchDeleteIfUnmodifiedSince',
                    error: err,
                });
                return callback(err);
            }
            log.debug('batch delete if-unmodified-since operation successful');
            return callback();
        });
}
...
function batchDelete(request, response, log, callback) {
    return _getRequestPayload(request, (err, payload) => {
        ...
        if (request.headers['if-unmodified-since']) {
            return _batchDeleteIfUnmodifiedSince(
                request, response, locations, log, callback);
        }
...
function routeBackbeat(clientIP, request, response, log) {
    ...
    const useMultipleBackend = request.resourceType.includes('multiplebackend');
